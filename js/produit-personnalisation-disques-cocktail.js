/**
 * Personnalisation disques à cocktail — 6 formes sur feuille A4
 */
(function () {
    'use strict';

    var modal = document.getElementById('modal-personnalisation-disques-cocktail');
    if (!modal) {
        return;
    }
    ...
    var EDGE_MARGIN_CM = 0.4;
    var MIN_GAP_CM = 0.5;
    var HEART_DRAW_SCALE = 1.06;

    var btnClose = document.getElementById('disques-cocktail-modal-close');
    var btnCancel = document.getElementById('disques-cocktail-cancel');
    var btnValidate = document.getElementById('disques-cocktail-validate');
    var fileInput = document.getElementById('disques-cocktail-image-input');
    var uploadLabel = document.getElementById('disques-cocktail-upload-label');
    var uploadText = document.getElementById('disques-cocktail-upload-text');
    var filenameEl = document.getElementById('disques-cocktail-upload-filename');
    var canvas = document.getElementById('disques-cocktail-preview-canvas');
    var previewViewport = document.getElementById('disques-cocktail-preview-viewport');
    var imageManipulator = document.getElementById('disques-cocktail-image-manipulator');
    var imageManipBox = document.getElementById('disques-cocktail-image-manip-box');
    var imageHintEl = document.getElementById('disques-cocktail-image-hint');
    var imageResetBtn = document.getElementById('disques-cocktail-image-reset');
    var imageDeleteBtn = document.getElementById('disques-cocktail-image-delete');
    var dimDiameter = document.getElementById('disques-cocktail-dim-diameter');
    var dimDiameterVal = document.getElementById('disques-cocktail-dim-diameter-val');
    var paperInfoEl = document.getElementById('disques-cocktail-paper-info');
    var activeCircleLabel = document.getElementById('disques-cocktail-active-circle-label');
    ...
    var textManipBox = document.getElementById('disques-cocktail-text-manip-box');
    var textEngine = null;
    if (window.PersoTextEngine) {
        textEngine = window.PersoTextEngine.create({
            modal: modal,
            prefix: 'disques-cocktail',
            getShape: function () {
            ...
            clientToCanvas: function (clientX, clientY) {
                return clientToCanvas(clientX, clientY) || { x: 0, y: 0 };
            },
    ...
    function bindModalDeselect() {
        modal.addEventListener('pointerdown', function (event) {
            ...
            if (event.target.closest(
                '#disques-cocktail-preview-canvas, .perso-text-manipulator, .perso-image-manipulator, ' +
    ...
    /**
     * Grille 2×3 sur toute la feuille : 2 disques en haut, 2 au centre, 2 en bas.
     * Le diamètre affiché suit le paramètre cm (max 8) ; l’espace restant est réparti en marges/gaps.
     */
    function getCircleLayouts(paperBounds) {
        ...
        var minGapPx = MIN_GAP_CM * paperBounds.pxPerCm;

        var maxDByW = (usableW - (COLS - 1) * minGapPx) / COLS;
        var maxDByH = (usableH - (ROWS - 1) * minGapPx) / ROWS;
        var maxDFit = Math.max(1, Math.min(maxDByW, maxDByH));
        var requestedPx = state.diameterCm * paperBounds.pxPerCm;
        var dPx;
        if (state.shape === 'heart') {
            dPx = Math.min(requestedPx, maxDFit / HEART_DRAW_SCALE) * HEART_DRAW_SCALE;
        } else {
            dPx = Math.min(requestedPx, maxDFit);
        }

        var gapX = COLS > 1 ? Math.max(minGapPx, (usableW - COLS * dPx) / (COLS - 1)) : 0;
        var gapY = ROWS > 1 ? Math.max(minGapPx, (usableH - ROWS * dPx) / (ROWS - 1)) : 0;
        var startX = paperBounds.x + edge;
        var startY = paperBounds.y + edge;

        var layouts = [];
        var idx = 0;
        for (var r = 0; r < ROWS; r++) {
            for (var c = 0; c < COLS; c++) {
                var x = startX + c * (dPx + gapX);
                var y = startY + r * (dPx + gapY);
                var cell = {
                    index: idx,
                    x: x,
                    y: y,
    ...
    function updateModeUi() {
        ...
        if (uploadText) {
            uploadText.textContent = state.imageMode === 'shared'
                ? 'Importer une image (tous les disques)'
                : 'Importer pour le disque sélectionné';
        }
        if (activeCircleLabel) {
    ...
    function getActiveContext() {
        ...
        return {
            ...
            btnOpen: document.getElementById('btn-personnaliser-disques-cocktail') || document.querySelector('.js-open-disques-cocktail-perso-modal')
        };
    }
    ...
    function canvasToFile(callback) {
        ...
        canvas.toBlob(function (blob) {
            ...
            callback(new File([blob], 'personnalisation-disques-cocktail.png', { type: 'image/png' }));
        }, 'image/png', 0.92);
    }
    ...
    document.addEventListener('click', function (event) {
        var openBtn = event.target.closest('.js-open-disques-cocktail-perso-modal, #btn-personnaliser-disques-cocktail');
        if (openBtn) {
    ...
    if (dimDiameter) {
        dimDiameter.addEventListener('input', function () {
            state.diameterCm = clamp(parseFloat(dimDiameter.value) || 8, 1, 8);
            if (dimDiameterVal) {
                dimDiameterVal.textContent = formatCm(state.diameterCm);
            }
            renderPreview();
    ...
    if (canvas) {
        ...
        canvas.addEventListener('pointerup', finishImageManipDrag);
        canvas.addEventListener('pointercancel', finishImageManipDrag);
    ...
    if (imageManipBox) {
        imageManipBox.addEventListener('pointerdown', function (event) {
            ...
            var pt = clientToCanvas(event.clientX, event.clientY);
            if (!pt) {
                return;
            }
            event.preventDefault();
