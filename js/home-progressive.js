...
(function () {
  ...
  function initLazySections() {
    ...
    // Accueil : première section dès que le navigateur est libre (sans bloquer le 1er rendu)
    if (document.body.classList.contains('page-home')) {
      var firstHome = getNextLazyContainer();
      if (firstHome) {
        var startFirst = function () {
          loadLazySection(firstHome);
        };
        if (window.requestIdleCallback) {
          window.requestIdleCallback(startFirst, { timeout: 400 });
        } else {
          setTimeout(startFirst, 60);
        }
        return;
      }
    }
