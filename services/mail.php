...
function mail_create_instance() {
    ...
    // Hébergement partagé : certificat émis pour *.web-hosting.com au lieu de mail.domaine.com
            ...
            // Évite les attentes interminables si le SMTP ne répond pas
            $mail->Timeout = min(15, max(5, (int) ($smtp['timeout'] ?? 15)));
            break;
        case 'sendmail':
            $mail->isSendmail();
            break;
...
function mail_send_contact($nom, $email, $sujet, $message) {
    ...
    if ($result['success']) {
        ...
        mail_send($email, "Confirmation - Sugar Paper", "<p>" . htmlspecialchars($reply) . "</p>", true);
    }
    return $result;
}
...
function mail_send_reset_link($email, $reset_link, $type = 'user') {
    ...
    $body .= '<p style="font-size: 12px; color: #999;">Sugar Paper - Produits naturels</p>';
    $body .= '</div>';
