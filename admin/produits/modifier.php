...
<head>
    ...
    <style>
        ...
        .options-input { flex: 1; min-width: 150px; padding: 10px 14px; border: 2px solid #e8e8e8; border-radius: 8px; font-size: 14px; }
        ...
        .options-surcharge { width: 90px; padding: 8px 10px; border: 2px solid #e8e8e8; border-radius: 8px; font-size: 13px; }
        .option-tag .tag-surcharge { font-size: 11px; color: #666; margin-left: 4px; }
        .variantes-container { margin-bottom: 15px; }
        .variante-item { margin-bottom: 12px; padding: 12px; background: #f9f9f9; border-radius: 8px; border: 1px solid #eee; }
        .variante-row { display: flex; align-items: center; gap: 12px; flex-wrap: wrap; }
        .variante-nom { flex: 1; min-width: 150px; padding: 10px 14px; border: 2px solid #e8e8e8; border-radius: 8px; }
        .variante-prix, .variante-prix-promo { width: 100px; padding: 10px 14px; border: 2px solid #e8e8e8; border-radius: 8px; }
        ...
        .btn-remove-variante { width: 32px; height: 32px; border: none; background: #c00; color: #fff; border-radius: 50%; cursor: pointer; font-size: 18px; line-height: 1; }
        .btn-remove-variante:hover { background: #a00; }
        .btn-add-variante { padding: 10px 18px; background: #918a44; color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 14px; display: inline-flex; align-items: center; gap: 8px; }
    ...
    <div class="form-container">
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <p style="font-size: 12px; color: #666; margin-bottom: 12px;">Ajoutez des variantes avec un nom, un prix et une image différents. Les options couleur, poids et taille s'appliquent aussi aux variantes.</p>
                        ...
                        <?php foreach ($variantes as $idx => $var): ?>
                            <div class="variante-item" data-index="<?php echo $idx; ?>">
                                <div class="variante-row">
                                    ...
                                    <input type="text" name="variantes_nom[]" placeholder="Nom de la variante" class="variante-nom" value="<?php echo htmlspecialchars($var['nom']); ?>">
                                    <input type="number" name="variantes_prix[]" placeholder="Prix FCFA" min="0" step="0.01" class="variante-prix" value="<?php echo htmlspecialchars($var['prix']); ?>">
                                    <input type="number" name="variantes_prix_promo[]" placeholder="Prix promo" min="0" step="0.01" class="variante-prix-promo" value="<?php echo $var['prix_promotion'] ? htmlspecialchars($var['prix_promotion']) : ''; ?>">
                                    <div class="variante-image-wrap">
                                        <div class="variante-image-area">
                                            ...
                                            <span class="variante-image-label" <?php echo $var['image'] ? 'style="display: none;"' : ''; ?>><i class="fas fa-image"></i> <?php echo $var['image'] ? 'Changer' : 'Image'; ?></span>
                                            <img class="variante-preview-img" src="<?php echo $var['image'] ? '../../upload/' . htmlspecialchars($var['image']) : ''; ?>" alt="" <?php echo $var['image'] ? '' : 'style="display: none;"'; ?>>
                                        </div>
                                    </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <div class="options-add-block options-with-surcharge">
                        ...
                        <input type="hidden" name="poids" id="poids-hidden" value="<?php echo htmlspecialchars($produit['poids'] ?? ''); ?>">
                    </div>
                    <small style="color: #666; font-size: 12px; display: block; margin-top: 5px;">Poids + montant optionnel (ex: 1kg + 300). Laissez vide pour 0.</small>
                </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <div class="options-add-block options-with-surcharge">
                        ...
                        <input type="hidden" name="taille" id="taille-hidden" value="<?php echo htmlspecialchars($produit['taille'] ?? ''); ?>">
                    </div>
                    <small style="color: #666; font-size: 12px; display: block; margin-top: 5px;">Taille + montant optionnel (ex: L + 200). Laissez vide pour 0.</small>
                </div>
            </div>
